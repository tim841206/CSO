...
function Export($title, $resource) {
    ...
    if ($title == "SLSMAS") {
        ...
        while($SLSMAS = mysql_fetch_array($resource)){
            echo '<tr><td>'.$SLSMAS['SALPERNO'].'</td><td>'.$SLSMAS['SALPERNM'].'</td><td>'.$SLSMAS['EMPNO'].'</td><td>'.$SLSMAS['COMRATE'].'</td><td>'.$SLSMAS['SALEAMTYTD'].'</td><td>'.$SLSMAS['SALEAMTSTD'].'</td><td>'.$SLSMAS['SALEAMTMTD'].'</td></tr>';
        }
    }
    elseif ($title == "CUSMAS") {
        echo '<tr><th>顧客</th></tr>';
    ...
    elseif ($title == "CUSADD") {
        ...
        while($CUSADD = mysql_fetch_array($resource)){
            echo '<tr><td>'.$CUSADD['CUSNO'].'</td><td>'.$CUSADD['ADDNO'].'</td><td>'.$CUSADD['ADD_1'].'</td><td>'.$CUSADD['ADD_2'].'</td><td>'.$CUSADD['ADD_3'].'</td><td>'.$CUSADD['CITY'].'</td><td>'.$CUSADD['COUNTY'].'</td><td>'.$CUSADD['COUNTRY'].'</td><td>'.$CUSADD['ZCODE'].'</td><td>'.$CUSADD['CNTPER'].'</td><td>'.$CUSADD['TEL'].'</td><td>'.$CUSADD['FAX'].'</td><td>'.$CUSADD['EMAIL'].'</td></tr>';
        }
    }
    elseif ($title == "CUSREGION") {
        ...
        while($CUSREGION = mysql_fetch_array($resource)){
            echo '<tr><td>'.$CUSREGION['REGIONNO'].'</td><td>'.$CUSREGION['CHANNELNO'].'</td><td>'.$CUSREGION['CHANNELNM'].'</td><td>'.$CUSREGION['COMPANYNO'].'</td><td>'.$CUSREGION['COMPANYNM'].'</td><td>'.$CUSREGION['DISTRICTNO'].'</td><td>'.$CUSREGION['DESCRIPTION'].'</td></tr>';
        }
    }
    elseif ($title == "CUSCITY") {
        ...
        while($CUSCITY = mysql_fetch_array($resource)){
            echo '<tr><td>'.$CUSCITY['CITYNO'].'</td><td>'.$CUSCITY['CITYNM'].'</td><td>'.$CUSCITY['REGIONNO'].'</td></tr>';
        }
    }
    echo '</table>';
    ?>
