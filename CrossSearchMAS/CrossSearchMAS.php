	...
	if (mysql_num_rows($resource) == 0) {
		...
		if ($title == "CUSADD") {
			...
			while ($CUSADD = mysql_fetch_array($resource)) {
				$table .= '<tr><td>'.$CUSADD['CUSNO'].'</td><td>'.$CUSADD['ADDNO'].'</td><td>'.$CUSADD['ADD_1'].'</td><td>'.$CUSADD['ADD_2'].'</td><td>'.$CUSADD['ADD_3'].'</td><td>'.$CUSADD['CITY'].'</td><td>'.$CUSADD['COUNTY'].'</td><td>'.$CUSADD['COUNTRY'].'</td><td>'.$CUSADD['ZCODE'].'<td>'.$CUSADD['CNTPER'].'</td><td>'.$CUSADD['TEL'].'</td><td>'.$CUSADD['FAX'].'</td><td>'.$CUSADD['EMAIL'].'</td><td>'.$CUSADD['CREATEDATE'].'</td><td>'.$CUSADD['UPDATEDATE'].'</td></tr>';
			}
		}
		elseif ($title == "CUSCITY") {
			...
			while($CUSCITY = mysql_fetch_array($resource)){
				$table .= '<tr><td>'.$CUSCITY['CITYNO'].'</td><td>'.$CUSCITY['CITYNM'].'</td><td>'.$CUSCITY['REGIONNO'].'</td><td>'.$CUSCITY['CREATEDATE'].'</td><td>'.$CUSCITY['UPDATEDATE'].'</td></tr>';
			}
		}
		elseif ($title == "CUSMAS") {
			$table .= '<tr><th>顧客編號</th><th>顧客名稱</th><th>地址 1</th><th>地址 2</th><th>地址 3</th><th>所屬城市</th><th>所屬縣市</th><th>所屬國家</th><th>郵遞區號</th><th>接觸人員</th><th>電話</th><th>傳真</th><th>電子信箱</th><th>網址</th><th>所屬銷售員編號</th><th>預設運送地編號</th><th>預設帳單地編號</th><th>年結帳至今銷售額</th><th>季結帳至今銷售額</th><th>月結帳至今銷售額</th><th>應收帳款</th><th>30~60天應收帳款</th><th>60~90天應收帳款</th><th>90~120天應收帳款</th><th>超過120天應收帳款</th><th>特殊要求</th><th>信用狀態</th><th>統一編號</th><th>建立日期</th><th>最後修改日期</th></tr>';
			while ($CUSMAS = mysql_fetch_array($resource)) {
				$table .= '<tr><td>'.$CUSMAS['CUSNO'].'</td><td>'.$CUSMAS['CUSNM'].'</td><td>'.$CUSMAS['ADD_1'].'</td><td>'.$CUSMAS['ADD_2'].'</td><td>'.$CUSMAS['ADD_3'].'</td><td>'.$CUSMAS['CITY'].'</td><td>'.$CUSMAS['COUNTY'].'</td><td>'.$CUSMAS['COUNTRY'].'</td><td>'.$CUSMAS['ZCODE'].'</td><td>'.$CUSMAS['CNTPER'].'</td><td>'.$CUSMAS['TEL'].'</td><td>'.$CUSMAS['FAX'].'</td><td>'.$CUSMAS['EMAIL'].'</td><td>'.$CUSMAS['WSITE'].'</td><td>'.$CUSMAS['SALPERNO'].'</td><td>'.$CUSMAS['DFSHIPNO'].'</td><td>'.$CUSMAS['DFBILLNO'].'</td><td>'.$CUSMAS['SALEAMTYTD'].'</td><td>'.$CUSMAS['SALEAMTSTD'].'</td><td>'.$CUSMAS['SALEAMTMTD'].'</td><td>'.$CUSMAS['CURAR'].'</td><td>'.$CUSMAS['AR30_60'].'</td><td>'.$CUSMAS['AR60_90'].'</td><td>'.$CUSMAS['AR90_120'].'</td><td>'.$CUSMAS['M120AR'].'</td><td>'.$CUSMAS['SPEINS'].'</td><td>'.$CUSMAS['CREDITSTAT'].'</td><td>'.$CUSMAS['TAXID'].'</td><td>'.$CUSMAS['CREATEDATE'].'</td><td>'.$CUSMAS['UPDATEDATE'].'</td></tr>';
			}
		}
		elseif ($title == "CUSADDCITY") {
			...
			while ($CUSADDCITY = mysql_fetch_array($resource)) {
				$table .= '<tr><td>'.$CUSADDCITY['CUSNO'].'</td><td>'.$CUSADDCITY['CUSNM'].'</td><td>'.$CUSADDCITY['ADDNO'].'</td><td>'.$CUSADDCITY['CITYNO'].'</td><td>'.$CUSADDCITY['CREATEDATE'].'</td><td>'.$CUSADDCITY['UPDATEDATE'].'</td></tr>';
			}
		}
		$table .= '</table>';
		return $table;
